...
joined_data %>% 
  ggplot(aes(x = NHWA, y = vote_difference, size = TOT_POP, color = victor)) + 
    ...
    theme_minimal() +
    theme(legend.position = c(.8, 1.05),
...
joined_data %>% 
  ggplot(aes(x = NHWA, y = vote_difference, size = TOT_POP, color = victor)) + 
    ...
    theme_minimal() +
    theme(legend.position = c(.8, 1.15),
            legend.direction = "horizontal",
            legend.title = element_blank()) +
    scale_color_brewer(type = "qual", palette = "Set1", direction = -1) +
    scale_x_continuous(labels = scales::percent) +
...
```{r, fig.width=8}
joined_data %>% 
  ggplot(aes(x = bachelorsOrMore, y = vote_difference, size = TOT_POP, color = victor)) + 
    geom_point(alpha = 1/3) + 
    theme_minimal() +
    theme(legend.position = c(.8, 1.08),
          legend.direction = "horizontal",
          legend.title = element_blank()) +
    scale_color_brewer(type = "qual", palette = "Set1", direction = -1) +
    scale_x_continuous(labels = scales::percent) +
